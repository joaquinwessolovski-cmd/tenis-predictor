...
with open('tennis_model_step2.pkl', 'rb') as f:
...
X = pd.DataFrame(features_list, columns=['A_elo', 'B_elo', 'A_surf_elo', 'B_surf_elo', 'A_h2h', 'B_h2h',
...
print(f"\n=== Overall ===\nPrecision: {acc:.4f} en {len(df)} partidos")

res_df = pd.DataFrame(meta)
res_df['correct'] = (np.array(targets) == preds)
