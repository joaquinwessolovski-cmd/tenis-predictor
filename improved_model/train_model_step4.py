...
def main():
    ...
    xgb_model = xgb.XGBClassifier(eval_metric='logloss', colsample_bytree=0.66, learning_rate=0.041, max_depth=5, n_estimators=137, subsample=0.73)
    rf_model = RandomForestClassifier(n_estimators=150, max_depth=7, random_state=42, n_jobs=-1)
    lr_model = make_pipeline(StandardScaler(), LogisticRegression(C=0.1, max_iter=500))
    ...
    with open('tennis_model_step4.pkl', 'wb') as f:
        pickle.dump({
            'model': model,
            'elo_sys': elo_sys,
    ...
    with open('player_names_step4.pkl', 'wb') as f:
